...
function askToServerForTableInformation(tableId){
	...
	if(window.IsLoggedInVar && checkConnection()){
		askToServerForTableInformation2(tableId);
	}else{
		setTimeout(function(){askToServerForTableInformation(tableId)},100);
	}
}
function askToServerForTableInformation2(tableId){
	var xmlhttp;
	 	  ...
	 	  if ((xmlhttp.readyState==4 && xmlhttp.status==200) ||  (xmlhttp.readyState==4 && xmlhttp.status==422) ||  (xmlhttp.readyState==4 && xmlhttp.status==401)||  (xmlhttp.readyState==4 && xmlhttp.status==400))
	    {
			...
			if(IsJsonString(jsonStr) && checkConnectionLoggedIn(xmlhttp)){ // Me fijo si dio un error, en el caso de que de le sigo mandando
				...
				setTimeout(function(){askToServerForTableInformation2(tableId)}, 500);
			}			
			return true;
...
function generateTabsWithData(openTable){
	if(hasDfInformation(openTable.description)){
		var dfHtml='<li role="presentation" class="'+showIfActive(0)+'"><a href="#InformationTab0" aria-controls="tab1" role="tab" data-toggle="tab" class="trn">DATOS</a></li>';
	}else{
		var dfHtml=' ';
	}
	return '<ul class="nav nav-tabs" role="tablist">'+dfHtml+'<li role="presentation" class="'+showIfActive(1)+'"><a href="#InformationTab1" aria-controls="tab1" role="tab" data-toggle="tab" class="trn">PUNTAJES</a></li><li role="presentation" class="'+showIfActive(2)+'"><a href="#InformationTab2" aria-controls="tab2" role="tab" data-toggle="tab" class="trn">Premios</a></li><li role="presentation" class="'+showIfActive(3)+'"><a href="#InformationTab3" aria-controls="tab3" role="tab" data-toggle="tab" class="trn">'+positionsOrPlayingText(openTable)+'</a></li></ul>';
}
...
function informationCostOfTable(openTable){ // editar
	if(window.showTableInformatioType=="training"){
		return "<span class='trn'>Gratis</span>";
	}else{
...
function showArrayOfIncidencesAndPoints(openTable){
	var showTablePoints='<tr> <td class="trn">Disparo al arco</td><td>'+(parseInt(openTable.table_rules.shots_on_goal)+parseInt(openTable.table_rules.shots))+' Pts</td></tr><tr> <td class="trn">Disparo al palo</td><td>'+(parseInt(openTable.table_rules.shots_to_the_post)+parseInt(openTable.table_rules.shots))+' Pts</td></tr><tr> <td class="trn">Disparo afuera</td><td>'+(parseInt(openTable.table_rules.shots_outside)+parseInt(openTable.table_rules.shots))+' Pts</td></tr><tr> <td class="trn">Goles</td><td>'+(parseInt(openTable.table_rules.scored_goals)+parseInt(openTable.table_rules.shots))+' Pts</td></tr><tr> <td class="trn">Goles (DEF)</td><td>'+(parseInt(openTable.table_rules.scored_goals)+parseInt(openTable.table_rules.shots)+parseInt(openTable.table_rules.defender_scored_goals))+' Pts</td></tr><tr> <td class="trn">Goles (ARQ)</td><td>'+(parseInt(openTable.table_rules.scored_goals)+parseInt(openTable.table_rules.shots)+parseInt(openTable.table_rules.goalkeeper_scored_goals))+' Pts</td></tr><tr> <td class="trn">Tarjeta amarilla</td><td>('+openTable.table_rules.yellow_cards+') Pts</td></tr><tr> <td class="trn">Tarjeta roja</td><td>('+openTable.table_rules.red_cards+') Pts</td></tr><tr> <td class="trn">Pases correctos</td><td>'+openTable.table_rules.right_passes+' Pts</td></tr><tr> <td class="trn">Pases incorrectos</td><td>('+openTable.table_rules.wrong_passes+') Pts</td></tr><tr> <td class="trn">Faltas</td><td>('+openTable.table_rules.faults+') Pts</td></tr><tr> <td class="trn">Recuperaciones</td><td>'+openTable.table_rules.recoveries+' Pts</td></tr><tr> <td class="trn">Asistencias</td><td>'+openTable.table_rules.assists+' Pts</td></tr><tr> <td class="trn">Fuera de juego</td><td>('+openTable.table_rules.offside+') Pts</td></tr><tr> <td class="trn">Atajadas</td><td>'+openTable.table_rules.saves+' Pts</td></tr><tr> <td class="trn">Penal errado</td><td>('+openTable.table_rules.missed_penalties+') Pts</td></tr><tr> <td class="trn">Penal atajado (ARQ)</td><td>'+openTable.table_rules.saved_penalties+' Pts</td></tr><tr> <td class="trn">Gol al arquero(ARQ)</td><td>('+openTable.table_rules.missed_saves+') Pts</td></tr><tr> <td class="trn">Valla invicta (ARQ)</td><td>'+openTable.table_rules.undefeated_goal+' Pts</td></tr><tr> <td class="trn">Valla invicta (DEF)</td><td>'+openTable.table_rules.undefeated_defense+' Pts</td></tr><tr> <td class="trn">Equipo ganador</td><td>'+openTable.table_rules.winner_team+' Pts</td></tr>';

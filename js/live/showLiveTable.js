...
function parseShowUsersComplete(user){
	...
	return '<div class="row players-list-item vertical-align color-player-list '+oddEven+'"> <div class="col-xs-2 col-sm-2"><p class="text-block-style2" id="live-userTablePositionShow-'+user.id+'">'+user.userPosition+'</p></div><div class="col-xs-4 col-sm-4 player-name nopadding"> <p><strong>'+nick+'</strong></p></div><div class="col-xs-2 col-sm-2 nopadding" id="live-userTableCoinsShow-'+user.id+'"> '+coinForPositionTableLive(user.userPosition)+'</div><div class="col-xs-2 col-sm-2 nopadding" id="live-userTablePointsShow-'+user.id+'"> '+user.playersPoints+' Pts </div><div class="col-xs-2 col-sm-2 text-right"> <button type="button" onClick="changeArrow(this);" class="btn btn-live" data-toggle="collapse" data-target="#live-userPlayersContainer-'+user.id+'" ><i class="fa fa-chevron-down" aria-hidden="true"></i></button> </div></div><div id="live-userPlayersContainer-'+user.id+'" class="collapse">'+collapseHtml+'</div>';
}
...
function parseShowPlayersComplete(player,number){
	...
	return '<div class="row players-list-item vertical-align color-player-list '+oddEven+'"> <div class="col-xs-2 col-sm-2"><img class="team-logo" src="img/icons/team-logo/'+player.playerTeamId+'.gif"></div><div class="col-xs-6 col-sm-6 player-name"> <p><strong>'+player.playerName+'</strong></p><p>'+player.playerPosition+'</p></div><div class="col-xs-2 col-sm-2 nopadding" id="live-playerTablePointsShow-'+player.id+'"> '+player.playerPoints+' Pts </div><div class="col-xs-2 col-sm-2 text-right"> <button type="button" onClick="changeArrow(this);" class="btn btn-live" data-toggle="collapse" data-target="#live-playerAllStatsContainer-'+player.id+'" ><i class="fa fa-chevron-down" aria-hidden="true"></i></button> </div></div><div id="live-playerAllStatsContainer-'+player.id+'" class="collapse">'+parseShowIncidences(player.playerStats)+'</div>';
}
		...
		for(stats in insidences){ // Los pases los sumo directamente para no ser confuso como se muestra
			if(insidences[stats].incidenciaId==180){rightPasses++;}
				...
				if(insidences[stats].incidenciaId==0){ // Equipo ganador y valla invicta no llevan minutos
					htmlToShow+='<div class="row players-list-item vertical-align color-player-list2 '+parseNumberToTextOddOrEven(countForEven)+'" > <div class="col-xs-2 col-sm-2"><img class="team-logo" src="img/icons/history/'+insidences[stats].name+'.png"></div><div class="col-xs-5 col-sm-5 player-name"> <p>'+parseStatsNameToShow(insidences[stats].name)+'</p></div><div class="col-xs-1 col-sm-1 text-right nopadding"> </div><div class="col-xs-4 col-sm-4"> <p class="text-right nomarging"> <span class="text-block-style2">'+parseStatsPointsToShow(insidences[stats].value)+'</span> <b>Pts</b></p></div></div>';
				}else{
					htmlToShow+='<div class="row players-list-item vertical-align color-player-list2 '+parseNumberToTextOddOrEven(countForEven)+'" > <div class="col-xs-2 col-sm-2"><img class="team-logo" src="img/icons/history/'+insidences[stats].name+'.png"></div><div class="col-xs-5 col-sm-5 player-name"> <p>'+parseStatsNameToShow(insidences[stats].name)+'</p></div><div class="col-xs-1 col-sm-1 text-right nopadding"> '+insidences[stats].minutes+'\' </div><div class="col-xs-4 col-sm-4"> <p class="text-right nomarging"> <span class="text-block-style2">'+parseStatsPointsToShow(insidences[stats].value)+'</span> <b>Pts</b></p></div></div>';
				}
				countForEven++;
			}
		}
		if(wrongPasses>0){// Sumo los pases correctos
			htmlToShow='<div class="row players-list-item vertical-align color-player-list2 '+parseNumberToTextOddOrEven(2)+'" > <div class="col-xs-2 col-sm-2"><img class="team-logo" src="img/icons/history/right_passes.png"></div><div class="col-xs-5 col-sm-5 player-name"> <p>'+parseStatsNameToShow("wrong_passes")+'</p></div><div class="col-xs-1 col-sm-1 text-right nopadding"> <b>'+wrongPasses+'</b> </div><div class="col-xs-4 col-sm-4"> <p class="text-right nomarging"> <span class="text-block-style2">'+parseStatsPointsToShow(window.liveTableOpen.table_rules.wrong_passes*wrongPasses)+'</span> <b>Pts</b></p></div></div>'+htmlToShow;
			countForEven++;
		}
		if(rightPasses>0){// Sumo los pases correctos
			if(wrongPasses>0){countForEven=1;}else{countForEven=2;}// Para que quede variado
			htmlToShow='<div class="row players-list-item vertical-align color-player-list2 '+parseNumberToTextOddOrEven(countForEven)+'" > <div class="col-xs-2 col-sm-2"><img class="team-logo" src="img/icons/history/right_passes.png"></div><div class="col-xs-5 col-sm-5 player-name"> <p>'+parseStatsNameToShow("right_passes")+'</p></div><div class="col-xs-1 col-sm-1 text-right nopadding"> <b>'+rightPasses+'</b> </div><div class="col-xs-4 col-sm-4"> <p class="text-right nomarging"> <span class="text-block-style2">'+parseStatsPointsToShow(window.liveTableOpen.table_rules.right_passes*rightPasses)+'</span> <b>Pts</b></p></div></div>'+htmlToShow;
			countForEven++;
		}
	}else{
...
function parseShowPlayersBase(player, number){
	return '<div class="row players-list-item vertical-align color-player-list2 '+parseNumberToTextOddOrEven(number)+'"> <div class="col-xs-2 col-sm-2"><img class="team-logo" src="img/icons/team-logo/'+player.playerTeamId+'.gif"></div><div class="col-xs-6 col-sm-6 player-name"> <p><strong>'+player.playerName+'</strong></p><p>'+player.playerPosition+'</p></div><div class="col-xs-4 col-sm-4"> <p class="text-right nomarging"> <span class="text-block-style2">'+player.playerPoints+'</span> <b>Pts</b></p></div></div>';
}

...
function checkIfUsersLinkHasInvitationCode(){
	// cnl user Id
	// cri request Id
	if(getQueryVariableTranslated("tkn")!=-1){
		askForInvitationIdFromRequest(getQueryVariableTranslated("tkn"));
		window.invitationTknId=getQueryVariableTranslated("tkn");
	}
}
	...
	var xmlhttp;
	 	 ...
	 	 if ((xmlhttp.readyState==4 && xmlhttp.status==200) ||  (xmlhttp.readyState==4 && xmlhttp.status==422))
	    {
			...
			if(IsJsonString(jsonStr)){ // Me fijo si dio un error, en el caso de que de le sigo mandando
				//window.invitationId=(JSON.parse(jsonStr)).id;// No hay que guardar nada, lo unico que hace es avisar en el servidor que entro
			}else{

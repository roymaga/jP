...
function bodyOfDetailHistory(historyMatch){
	return '<div class="container container-full historial-detalle"><div class="container container-title bg-color2"><h3>'+historyMatch.table.title+'</h3></div><div class="container head-detalle text-center text-color2 text-uppercase"><div class="row text-center"><div class="col-xs-6 col-sm-6"><h1>'+historyMatch.table.position+'</h1><h5>Mi posición en este partido</h5></div><div class="col-xs-6 col-sm-6"><h1>'+historyMatch.earn_coins+' <img src="img/icons/coins/coins.png" style="margin-right: 0px;margin-top: -10px;margin-bottom: -3px;margin-left: 5px;width: 30px;"></h1><h5>Monedas Obtenidas</h5></div></div></div><div class="container bg-color5 row" style="margin: 0px;padding: 0px;"><a class="btn btn-primary btn-style3 full-width" onClick="openTableInformation(\''+historyMatch.table.id+'\');"><i class="fa fa-info-circle" aria-hidden="true"></i> Detalle del partido</a></div><div class="container list-style2 text-color2">'+jugadoresParaElHistorial(historyMatch.players,historyMatch.table.id)+'</div></div>';
}
function jugadoresParaElHistorial(playersSel,tableId){
	contentAgregarPlayer='';
	for(a in playersSel){
	contentAgregarPlayer+='<div class="row vertical-align bg-color5 text-color1"><div class="col-xs-1 col-sm-1" style="padding: 0px; width: 25px; margin-right: 4px;"><img src="'+playerGetImage(playersSel[a].id)+'" class="player-profile-pic small"></div><div class="col-xs-5 col-sm-5" style="padding: 0px;"><p>'+playersSel[a].first_name+' '+playersSel[a].last_name+'</p></div><div class="col-xs-2 col-sm-2" style="padding-right: 0px;"><p>'+playersSel[a].points+' Pts</p></div><div class="col-xs-2 col-sm-2" style="padding: 0px; width: 25px;"><img class="team-logo small" src="'+clubGetLogo(playersSel[a].team_id)+'"></div><div class="col-xs-2 col-sm-2"><a onClick="detallesHistoricosPuntaje('+playersSel[a].id+','+tableId+');" type="button" class="btn btn-default btn-style4 xsmall">Detalle</a></div></div>';
	}
...
function contenidoDelDetalleHistoricoSumado(){
	...
	textDetailPlayer+='<div class="container"><div class="line-divider"></div></div><div class="container stats1 text-center"><h4>Tarjetas</h4><img src="img/cards.jpg"><div class="row"><div class="col-xs-6 col-sm-6"><h2 class="color2">'+window.statsFromPlayer.stats.yellow_cards+'</h2><p>Amarillas</p></div><div class="col-xs-6 col-sm-6"><h2 class="color4">'+window.statsFromPlayer.stats.red_cards+'</h2><p>Rojas</p></div></div><h5 class="boxed">Puntos por tarjetas: '+scoreCards+'</h5></div>'; // detail cards
	textDetailPlayer+='<div class="container"><div class="line-divider"></div></div><div class="container stats1 text-center"><h4>Pases</h4><div class="grafico-pases"><div class="correcto" style="width:'+porcentualRightPases(window.statsFromPlayer.stats.right_passes,window.statsFromPlayer.stats.wrong_passes)+';"></div></div><div class="row"><div class="col-xs-6 col-sm-6"><h2 class="color1">'+window.statsFromPlayer.stats.right_passes+'</h2><p>Correctos</p></div><div class="col-xs-6 col-sm-6"><h2 class="color4">'+window.statsFromPlayer.stats.wrong_passes+'</h2><p>Incorrectos</p></div></div><h5 class="boxed">Puntos por pases: '+scorePases+'</h5></div><div class="container"><div class="line-divider"></div></div></div>'; // detail pases y cierre del principio
